...
let eval rules query =
  ...
  let rec backchain k env goals =
    match goals with
    | [] -> print_and_ask env
    | (BuiltinPredicate builtin) :: r -> begin
      match evaluate_builtin_predicate env builtin with
      | true, env -> backchain k env r
      | false, env -> false
      end
    | g :: r -> begin
      List.exists (fun kermit ->
        let (predicate, additionnal_goals), k' = rename_rule k kermit in
        try
          let env' = Unification.unify_predicates env (predicate, g) in
          backchain k' env' (additionnal_goals @ r)
